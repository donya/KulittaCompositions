...
Last modified: 18-Aug-2014 (documentation fix added 21-Apr-2015)
...
> import GrammarMonad hiding (choose)
> import Euterpea
> import PostProc
> import Constraints
...
> findInds :: [(String, Term CType)] -> Term CType -> [[(Int, Int)]]
> findInds e t = case t of
>     Mod m t -> findInds e t -- no extra offsets to worry about
>     S [] -> []
>     S (s:ss) -> 
>            let vs1 = findInds e s
>                len = length $ toAbsChords (expand e s) Major
>                vs2 = findInds e (S ss)
>            in  vs1 ++ map (add' len) vs2 where
>                add' y = map (\(a,b) -> (a+y, b+y))
>     Let x a exp -> 
>         let e' = (x, expand e a):e
>             xCases = findIndsSub x e' exp
>         in  xCases : findInds e' a ++ findInds e' exp
>     _ -> [] 

The findIndsSub function looks for instances of a variable and
determines what indices they occupy.

> findIndsSub :: String -> [(String, Term CType)] -> Term CType -> [(Int, Int)]
> findIndsSub x e t = case t of
>     Let x' a exp -> 
>         let a' = expand e a -- to figure out sequence length
>         in  if x'==x then [] -- x has been redefined, no instances to see here
>             else findIndsSub x e a ++ findIndsSub x ((x',a'):e) exp
>     Var x' -> if x/=x' then [] else 
>         let v = maybe (error (x ++ " is undefined")) id $ lookup x e
>             len = length $ toAbsChords v Major
>         in  [(0, len-1)]
>     Mod m t' -> findIndsSub x e t' -- mods don't change dependencies of offsets
>     S [] -> [] -- no instances to worry about
>     S (s:ss) -> 
>         let len = length $ toAbsChords (expand e s) Major
>         in  findIndsSub x e s ++ add' (len) (findIndsSub x e (S ss)) where
>         add' y = map (\(a,b) -> (a+y, b+y))
>     NT v -> [] -- not a variable, so no instances to worry about
